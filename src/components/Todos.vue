<template>
  <div class="column justify-center items-center" id="row-container">
    <q-card class="my-card">
      ...
      <q-card-section style="text-align: center">
        <q-btn color="red" text-color="black" label="Remove All" style="margin-right: 10px"
    ...
    <q-dialog v-model="confirm" persistent>
      <q-card>
        <q-card-section class="row items-center">
          ...
          <span class="q-ml-sm">Are you sure remove all?</span>
        </q-card-section>
        <q-card-actions align="right">
          <q-btn flat label="Cancel" color="primary" v-close-popup />
          <q-btn flat label="Yes" color="primary" v-close-popup @click="handleRemoveAll" />
        </q-card-actions>
      </q-card>
    </q-dialog>
...
export default {
  ...
  methods: {
    ...
    handleClickDelete(id) {
      this.$api.removeForId(id).then(() => {}).catch((error) => {
        this.$log.debug(error);
